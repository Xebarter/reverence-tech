    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${isScrolled
            ? 'bg-white shadow-xl py-2' // Fully opaque white on scroll
            : 'bg-[#1C3D5A] py-5'       // Original navy blue
          }`}
      >
        {/* Contact Strip - Only visible when not scrolled */}
        <div
          ...
          <div className="flex justify-between items-center h-12">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <button
                ...
                className={`px-6 py-2.5 rounded-xl font-black text-sm transition-all duration-300 shadow-lg flex items-center gap-2 ${isScrolled
                    ? 'bg-[#1C3D5A] text-white hover:bg-yellow-500 hover:text-[#1C3D5A]'
                    : 'bg-yellow-400 text-[#1C3D5A] hover:bg-white'
                  }`}
              >
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-[85%] max-w-sm bg-white z-[110] shadow-2xl p-8 flex flex-col"
            >
              <div className="flex justify-between items-center mb-12">
                ...
                <button
                  onClick={() => setIsMenuOpen(false)}
                  className="p-3 bg-slate-100 rounded-2xl text-slate-900 active:scale-95 transition-transform"
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-3 flex-1">
                {['Home', 'Services', 'Blog', 'Careers'].map((item) => (
                  <button
                    ...
                    className="flex items-center justify-between w-full p-5 text-lg font-black text-[#1C3D5A] bg-slate-50 rounded-2xl border border-transparent hover:border-yellow-400 hover:bg-white transition-all group"
                  >
                    {item}
                    <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm group-hover:bg-yellow-400 transition-colors">
                      <ChevronRight size={18} className="text-[#1C3D5A]" />
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="mt-auto pt-8 border-t border-slate-100">
                <button
                  ...
                  className="w-full bg-[#1C3D5A] text-white py-5 rounded-2xl font-black text-lg flex items-center justify-center gap-3 shadow-xl hover:shadow-2xl active:scale-[0.98] transition-all"
                >
